...
#
# These complement `dispatch.jl` (JET): `@inferred` catches a return type that resolves to an
# abstract/`Union` type, which is the class of issue most directly responsible for allocations and
# dynamic dispatch on a hot path.

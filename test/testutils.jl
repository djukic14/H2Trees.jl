...
using H2Trees
using H2Trees: treetrait, isBlockTree, isnear, LevelIterator, TranslatingNodesIterator
using H2Trees: ParentUpwardsIterator
...
function testwellseparatedness(tree, ::Any)
    Threads.@threads for level in H2Trees.levels(tree)
        for trialnode in LevelIterator(tree, level)
            for testnode in LevelIterator(tree, level)
                ...
                for testparent in ParentUpwardsIterator(tree, testnode)
                    ...
                    trialparent = H2Trees.parent(tree, trialparent)
...
function testwellseparatedness(tree, ::isBlockTree)
    ...
    for level in H2Trees.levels(trialtree)
        for trialnode in LevelIterator(trialtree, level)
            for testnode in LevelIterator(testtree, level)
                ...
                for testparent in ParentUpwardsIterator(testtree, testnode)
                    ...
                    trialparent = H2Trees.parent(trialtree, trialparent)
                    trialparent == 0 && break
                ...
                if connections != 1
                    println(
                        "Detected connection error: parenttranslations: $parentranslations directtranslations: $directranslations for testnode $testnode and trialnode $trialnode on level $level",
                    )
